...
| | count | share |
| --- | --- | ---: |
| **(a) measurable here, just not done** | **136** | 55 % |
| **(b) not measurable here** | **31** | 12 % |
| **(c) the question does not hold** | **82** | 33 % |
| **classified rows** | **249** | |
...
> that agreement — made it `133`; `D.24`, `D.25` and `D.26`, all three found while closing the round rather than while sweeping for gaps, made it `136`, and the table above says `136`.** Both states are recorded
...
| 43.3 | CHECK 5's cost on a GitHub runner | **a** | 15 | L | **corrected 2026-08-22, not closed — the row was wrong in one half and right in the other.** The **self-test** step ran on the runner for `5ac5fd5` and passed. The step that runs CHECK 5 **against the real tree was SKIPPED**, behind CHECK 3's expected failure, so the axis has still never been executed against this repository's own comments — it first will when `F` adds the roadmap rows, and **`F` must read that run rather than assume it**. ⚠️ The self-test step is reported as **1s**, which is the Actions API's one-second timestamp resolution and therefore **a coarse upper bound, not a profiled cost**. ✅ **EXECUTED 2026-08-23, and read rather than assumed.** `CHECK 5` ran against the merged tree at `a921d50` — **124 tracked `.kt`/`.java` files, 5 migrations** — and returned `OK: no KDoc denies an index a migration creates`, exit `0`. ⛔ **A pass is not evidence until it is shown it could have failed**, so the control was run: restoring the pre-fix text of the two files from the red commit `5f03310` made it **fire, exit `1`**, naming `RecommendationQueries.kt:9` and `RecommendationService.kt:69` and the migration `V2__attempt_learner_time_index.sql` that creates the index each denies. The tree was restored to `a921d50` with zero uncommitted files. ⭐ **So the silence is a measurement.** The remaining `미측정` in this row is only the runner cost |
...
| D.24 | `.github/workflows/docs-consistency.yml` `CHECK 3`; `docs/roadmap.md` | **`CHECK 3` asks whether the roadmap NAMES a report, not whether it carries a row for it** | **a** | 60 | **H** | ⛔⭐ **Found by slice `F` while using it, with a control.** `F` could not obtain roadmap rows for `R39`–`R42` and wrote a single row saying so, naming all four in backticks. **`CHECK 3` then passed.** Control run 2026-08-23: with that row the check is green; delete it and the check reports `R39 R40 R41 R42` missing. **The note written to be honest about a gap is what hides the gap from the gate.** ⭐ The check's own failure text says *"a report it does not list is work that happened and that the map of the work does not know about"* — and a mention inside an apology satisfies it. This is `R17` §5's shape one gate over: a check that is right about its literal predicate and wrong about the property it is quoted as establishing. **The errand is to require a row whose first cell is the id**, not a substring match over the whole file ⭐⭐ **`R17` §5 keeps this finding and its scope widens.** It was filed as a limitation of **keyword checks over prose**. Three instances arrived within one hour of each other and **none of them is prose**: this CI check (a substring match satisfied by a mention), §8 of the slice contract (`D.25`, satisfied by titles), and `R44`'s *Green commit* field — which is satisfiable by **nothing**, because the SHA does not exist when the sentence naming it is written. **`R17` §5 is a property of specifications, not of keyword matching**: a requirement can be right about its literal predicate and wrong about the property it is quoted as establishing, and prose was only where it was noticed first |
| D.25 | the round-three slice contract, §5 *"the report contract"*; `_ROUND3-G-HANDOFF.md` §8 | **§8 asks a worker for *the exact sentences for the integrator* and is satisfiable by titles** | **a** | 45 | **H** | ⛔ **The failure is not slice G's alone and must not be filed as G's.** §8 states what it wants in prose and **nothing reads §8** — no gate, no template check, and no field a worker leaves visibly empty. G wrote four report titles under that heading and the contract was met as written. Slice E met it for **one** report of five and said so; the other four were promised *"when their reports do"* and **no mechanism existed to fire when that condition came true** (`D.3`'s shape). ⭐ **So the same clause under-delivered in two different slices, in two different ways, and neither was caught until an integrator tried to use the output.** ⛔ **A contract clause nothing reads is not a contract clause — it is a wish in a numbered list**, and §8 has exactly **two honest futures**. **① Check it**: a worker's §8 must contain a line that parses as a roadmap row, one block per report with each destination named, or the handoff is incomplete. **② Drop the pretense**: say §8 is advisory and the integrator composes — which makes the four marked rows in `docs/roadmap.md` the **normal case rather than the exception**, and is honest as long as it is stated. ⛔ **What must not happen is the third option, which is the one that produced this: leaving it written and unread.** ⭐⭐ **The price, counted in one pass, which turns the choice from a preference into a cost:**
**① `docs/roadmap.md`** — G's four rows composed by the integrator and marked weaker than the
other thirteen. **② `R0`** — E's and G's per-slice cells unfillable, so the scorecard shows **two
rows of four** and refuses to compare rounds. **③ this ledger** — `R34`, `R38` and `R42` filed no
row while carrying **32 §8 items** between them. **One unread clause damaged three artefacts in a
single round, and each was found by someone trying to use the output.** ⭐ It was met **once in
four**, by the slice that chose to — so **the clause did not fail because it was hard. It failed
because nothing read it.** The errand is to choose, not to write more prose asking harder |
| D.26 | `69a9107`; `_ROUND3-ORCHESTRATION.md` §7.3 | **a failing step's exit code is discarded by the composition that follows it** | **a** | 30 | **H** | ⛔ **Two authors, two repositories, the same day, both of whom knew better — which is what moves this from a slip to a property of the tool.** In `69a9107` a patch script asserted its anchor, failed, exited non-zero, **and the `git commit` chained after it ran anyway**, producing a commit message that described a tree that did not exist. The PO hit the identical shape hours earlier: an assertion failed, the next command regenerated an export from the unmodified file and **printed a success line**, caught only because a length looked wrong. ⭐⭐ **Put it beside `./gradlew --stop` (`D.21`), because they are the same missing check from opposite directions**: `--stop` returned **success for work it did not do** and nobody read the process table; `69a9107` returned **failure for work it did not do** and nobody read the exit code. ✅ **The answer is already in this round**, from the four-item floor check — *it reads the process table instead of believing an exit code*. The equivalent is **commit only after re-reading the file**: verify the tree, never the tool's report of the tree. The principle was here; it had simply never been pointed at `git commit` |
| D.27 | `_ROUND3-ORCHESTRATION.md`; `.study/14장` §4 | **the round's cheapest detector was the one nobody built — an integration pass is an instrument and was not modelled as one** | **c** | — | **H** | ⛔ **A judgement, filed as `(c)` so that nobody turns it into an errand.** Every instrument this round built on purpose is a **sweep**: nine traps, four slices, five gates, one ledger audit. **Slice `F` was designed to merge, not to detect** — and it produced `D.24`, `D.25`, `D.26` and `R44`'s unfillable field, **three of them while closing the round rather than while looking for anything.** ⭐⭐ A sweep can only find gaps of a shape it already models (`D.23`); **using the machinery for real work finds the shapes nobody modelled**, because the work does not care what the instrument expects. ⭐ **The consequence for round four, written while it is fresh:** the next yield probably does not come from adding sweeps. It comes from **using what exists for something real and watching what breaks under use.** ⛔ This is not a proposal to build an *integration-pass detector* — that would be a sweep again, and would find what it modelled |
...
| **(a) entries remaining** | **66** — ⚠️ **as of that round.** Recounted after round three: **136**, and the recount's unit and tree state are stated above rather than assumed |
